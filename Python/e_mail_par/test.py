...
class DochiMail(object):
    ...
    def setMail(self, subject, body, body_type='plain', attachments=None):
        ...
        self.TO_ADDRS_FMT = ",".join(
            [formataddr(addr) for addr in self.to_addrs])
        ...
        for attachment in attachments:
            ...
            try:
                ...
                attach_binary.add_header(
                    "Content-Disposition", 'attachment', filename=(self.ENCODING, '', filename))
    ...
    def send(self):
        ...
        try:
            ...
            # 메일 발송
            session.sendmail(self.FROM_ADDR_FMT,
                             self.TO_ADDRS_FMT, self.message.as_string())
...
# SMTP 계정 정보
username = "kimgs243@gmail.com"                 # Gmail 계정
password = "구글 앱 비밀번호"                   # Gmail 앱 비밀번호

# 메일 송/수신자 정보
from_addr = ('Google Dochi', 'kimgs243@gmail.com')  # 보내는 사람 주소. (이름, 이메일)
to_addrs = [                                     # 받는 사람 주소. (이름, 이메일)
    ('Naver Dochi', 'kimgs243@naver.com')
]
